...
public class CadModeloController implements Initializable {
    ...
    {
        ObservableList<String> filtros = FXCollections.observableArrayList("Todos","Código","Descrição","Marca");
    ...
    private void EvtAlterar(ActionEvent event) {
        if(tabela.getSelectionModel().getSelectedIndex() >= 0)
        {
            ...
            try {
                modelo = tabela.getSelectionModel().getSelectedItem();
                Parent root = FXMLLoader.load(getClass().getResource("/autorepaircontrol/interfaces/layout/CadModeloNovo.fxml"));
                Scene scene = new Scene(root);
                stage.setScene(scene);
    ...
    private void EvtExcluir(ActionEvent event) {
        ...
        alert.setContentText("Deseja excluir este modelo?");
    ...
    private void EvtBuscar(ActionEvent event) {
        ...
        if(cbFiltro.getSelectionModel().getSelectedItem().equals("Código"))
            filtro = "where mo_id = " + txtFiltro.getText();
        else if(cbFiltro.getSelectionModel().getSelectedItem().equals("Descrição"))
            filtro = "where upper(mo_desc) like '%" + txtFiltro.getText().toUpperCase()+"%'";
        else if(cbFiltro.getSelectionModel().getSelectedItem().equals("Marca"))
            filtro = "inner join marca_veiculo on modelo.mv_id = marca_veiculo.mv_id and upper(marca_veiculo.mv_desc) like '%" + txtFiltro.getText().toUpperCase()+"%'";
