...
import autorepaircontrol.controladoras.CtrGrupoServico;
...
public class CadGrupoServicoController implements Initializable {

    public static Object grupo;
    private CtrGrupoServico ctr;
    ...
    private void EvtNovo(ActionEvent event) {
        ...
        try {
            grupo = null;
            Parent root = FXMLLoader.load(getClass().getResource("/autorepaircontrol/interfaces/layout/CadGrupoServicoNovo.fxml"));
            Scene scene = new Scene(root);
            stage.setScene(scene);
            stage.setTitle("Cadastro de Grupo de Servico");
    ...
    private void EvtAlterar(ActionEvent event) {
        if(tabela.getSelectionModel().getSelectedIndex() >= 0)
        {
            ...
            try {
                ...
                Scene scene = new Scene(root);
                stage.setScene(scene);
                stage.setTitle("Alterar Grupo de Serviço");
                stage.initModality(Modality.APPLICATION_MODAL);
                stage.setResizable(false);
                stage.initStyle(StageStyle.UTILITY);
                stage.showAndWait();
    ...
    private void EvtBuscar(ActionEvent event) {
        ...
        if(cbFiltro.getSelectionModel().getSelectedItem().equals("Código"))
            filtro = "gs_id = " + txtFiltro.getText();
        else if(cbFiltro.getSelectionModel().getSelectedItem().equals("Descrição"))
            filtro = "upper(gs_desc) like '%" + txtFiltro.getText().toUpperCase()+"%'";
